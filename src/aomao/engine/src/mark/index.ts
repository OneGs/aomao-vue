...
class Mark implements MarkModelInterface {
  ...
  triggerMarkdown(event: KeyboardEvent) {
    const editor = this.editor;
    if (!isEngine(editor) || editor.options.markdown?.mode === false) return;
    const { change } = editor;
    const range = change.range.get();
    if (!range.collapsed || change.isComposing()) return;
    const { startNode, startOffset } = range.cloneRange().shrinkToTextNode();
    const node =
  ...
  findPlugin(mark: NodeInterface): MarkInterface | undefined {
    ...
    const result: MarkInterface | undefined = this.pluginCaches.get(key);
      ...
      if (isMarkPlugin(markPlugin) && mark.name === markPlugin.tagName) {
        ...
        if (
          Array.isArray(schemaRule)
            ? schemaRule.find((rule) => schema.checkNode(mark, rule.attributes))
  ...
  compare(
    ...
    if (
      ...
      return false;
    //样式属性长度不一致
    if (Object.keys(sourceStyles).length !== Object.keys(targetStyles).length)
      return false;
    //样式属性名称或值不一致
    if (
  ...
  splitOnCollapsed(
    ...
    const { node } = this.editor;
    if (
      (card.length === 0 || card.attributes(CARD_TYPE_KEY) !== "inline") &&
      (node.isMark(startNode) || (startParent && node.isMark(startParent)))
    ) {
      // 获取上面第一个非mark标签
      ...
      if (rightNodes.filter((child) => !child.isCursor()).length > 0) {
        ...
        // 右侧有文本
          ...
          while (
            ...
            //不是要移除的mark
            if (!removeMark.some((mark) => this.compare(markClone, mark))) {
        ...
        leftNodes.filter((childNode) => !childNode.isCursor()).length > 0
      ) {
          ...
          while (node) {
            ...
            if (node.textContent === atText) {
              ...
              if (
                ...
                (!alignNode && parent && nodeApi.isInline(parent))
              )
                break;
  ...
  splitOnExpanded(
    ...
    const cardStart = startNode.isCard()
      ? startNode
      : startNode.closest(CARD_SELECTOR);
    const cardEnd = endNode.isCard() ? endNode : endNode.closest(CARD_SELECTOR);
    if (
      !(
        (cardStart.length > 0 &&
          "inline" === cardStart.attributes(CARD_TYPE_KEY)) ||
        (cardEnd.length > 0 && "inline" === cardEnd.attributes(CARD_TYPE_KEY))
      )
    ) {
      //开始非mark标签父节点
  ...
  wrapByNode(
    ...
    if (nodeApi.isMark(node)) {
      ...
      while (
        ...
        targetChildrens.filter((child) => !child.isCursor()).length === 1 &&
        plugin &&
        curPlugin &&
        plugin.mergeLeval <= curPlugin.mergeLeval
      ) {
        const targetChild = targetChildrens.find((child) => !child.isCursor())!;
        ...
        if (this.compare(parent.clone(), mark, true)) {
          ...
          //插件一样，不合并，直接移除
            ...
            const targetMark = childs.find(
              (child) => !child.isCursor() && nodeApi.isMark(child)
            );
            if (targetMark) {
              parent = targetMark;
      ...
      const curPlugin = targetNode ? this.findPlugin(targetNode) : undefined;
      /**
        ...
        (child) => {
          ...
          if (isEditable || !child.equal(selection?.anchor!)) {
            if (started) {
              ...
              // 要包裹的节点是mark
              if (result && typeof result !== "boolean" && childIsMark) {
                if (
                  !isEditable &&
                  selection?.focus &&
                  result
  ...
  unwrapByNodes(
    ...
    const nodeApi = editor.node;
    nodes.forEach((node) => {
      removeMark = removeMark as NodeInterface | NodeInterface[] | undefined;
      if (
        !removeMark ||
        (!node.isCard() &&
          (Array.isArray(removeMark) ? removeMark : [removeMark]).some((m) =>
            this.compare(node, m)
          ))
      ) {
        nodeApi.unwrap(node);
      } else if (removeMark) {
        (Array.isArray(removeMark) ? removeMark : [removeMark]).forEach((m) => {
          const styleMap = m.css();
          Object.keys(styleMap).forEach((key) => {
            node.css(key, "");
          });
          //移除符合规则的class
          const removeClass = m.get<Element>()?.className.split(/\s+/);
          if (removeClass) {
            const { schema } = editor;
            const schemas = schema.find((rule) => rule.name === node.name);
            for (let i = 0; i < schemas.length; i++) {
              const schemaRule = schemas[i];
              removeClass.forEach((className) => {
                ...
                if (
                  schemaRule.attributes &&
                  schema.checkValue(schemaRule.attributes, "class", className)
                ) {
  ...
  unwrap(
    ...
    nodes.forEach((ancestor) => {
      ancestor.traverse(
        (child) => {
          if (!isEditable && (child.isText() || !selection?.anchor)) return;
          if (
            isEditable ||
            ...
            if (started) {
              if (isEditable || !child.equal(selection?.focus!)) {
                if (
                  node.isMark(child) &&
                  !child.isCard() &&
                  (isEditable || safeRange.isPointInRange(child, 0))
                ) {
                  markNodes.push(child);
                } else if (child.isCard()) {
                  const cardComponent = editor.card.find(child);
                  if (cardComponent && cardComponent.executeMark) {
                    (Array.isArray(removeMark)
                      ? removeMark
            ...
            if (
              ...
              while (parent && !parent.isCard() && node.isMark(parent)) {
                ...
                parent = parent.parent();
                if (parent && (parent.get<Node>()?.childNodes.length ?? 0) > 1)
                  break;
              }
            }
          }
  ...
  findMarks(range: RangeInterface) {
    ...
    const handleRange = (
      ...
      if (toStart) {
        ...
        const startNodeCloneChildren = startNodeClone.get<Node>()!.childNodes;
        const startOffsetNode = startNodeCloneChildren.item(startOffsetClone);
        ...
        if (
          ...
          (startNode.type !== Node.ELEMENT_NODE ||
            (startChildrenOffsetNode && !node.isBlock(startChildrenOffsetNode)))
        )
          return;
        cloneRange.select(startParent, true);
    ...
    if (!cloneRange.collapsed) {
      ...
      cloneRange.setStart(leftRange.startContainer, leftRange.startOffset),
        cloneRange.setEnd(rightRange.startContainer, rightRange.startOffset);
    }
              ...
              if (child[0] !== sc) {
                if (isBegin) {
                  ...
                  if (
                    ...
                    const cardComponent = editor.card.find(child);
                    if (cardComponent?.queryMarks) {
                      cardComponent.queryMarks().forEach((mark) => {
                        addNode(nodes, mark);
                      });
                    }
                  }
