...
class ChangeRange implements ChangeRangeInterface {
  ...
  select(range: RangeInterface, triggerSelect = true) {
    ...
    if (
      selection &&
      (range.collapsed ||
        (selection.rangeCount > 0 && !range.equal(selection.getRangeAt(0)))) &&
  ...
  focus(toStart?: boolean) {
    ...
    this.select(range);
    const editableElement = range.commonAncestorNode.closest(EDITABLE_SELECTOR);
