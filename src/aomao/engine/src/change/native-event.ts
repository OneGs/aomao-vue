...
class NativeEvent {
  ...
  repairInput(event: InputEvent, range: RangeInterface) {
    ...
    if (startNode.isText() && parent && node.isMark(parent)) {
      ...
      if (
        ...
        const markTops: Array<NodeInterface> = [];
        //循环查找
        while (markParent && node.isMark(markParent)) {
          ...
          if (markParent?.next() && markParentP && node.isMark(markParentP)) {
            break;
          }
        }
        ...
        markTops.forEach((markTop, index) => {
          ...
          while (!next && curNode) {
            //找到父节点
            const parent: NodeInterface | undefined = curNode.parent();
            //如果父节点是块级节点，就不找了
        ...
        if (markTops.length > 0) {
          const lastText = textNode.splitText(text.length - event.data.length);
          lastText.remove();
      ...
      //输入光标在mark节点开始位置
        ...
        while (markParent && node.isMark(markParent)) {
          ...
          //如果还有位于下方的同级节点，并且父级节点也是mark节点，说明当前光标不在末尾了
          const markParentP = markParent?.parent();
          if (markParent?.prev() && markParentP && node.isMark(markParentP)) {
            break;
          }
        }
      ...
      card.each((card) => {
        ...
        if (center && center.length > 0) {
          ...
          if (!isSelect && containsCard && selection.focusNode) {
            const focusCard = this.engine.card.find(selection.focusNode);
            if (focusCard) {
              isSingle =
  ...
  init() {
    ...
    change.event.onDocument("copy", (event) => {
      const range = change.range.get();
      if (!this.engine.container.contains(range.commonAncestorNode)) return;
    ...
    change.event.onPaste((data) => {
      ...
      if (this.engine.trigger("paste:event", data, source) === false) return;
      if (files.length === 0) {
        change.cacheRangeBeforeCommand();
        this.paste(source);
        ...
        if (markdown.mode !== false) {
          ...
          if (
            ...
            markdown.check(text ?? "", html ?? "").then((result) => {
              if (result) {
