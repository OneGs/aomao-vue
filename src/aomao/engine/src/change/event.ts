...
class ChangeEvent implements ChangeEventInterface {
  ...
  onInput(callback: EventListener) {
    ...
    this.onContainer("compositionstart", (event) => {
      ...
      if (!range.collapsed) {
        ...
        if (
          (node.isCustomize(startBlock) || node.isCustomize(endBlock)) &&
      ...
      if (!this.isComposing) {
        ...
        if (isAndroid && androidCustomeListComposingNode) {
          ...
          if (range.collapsed && startNode?.isText()) {
            ...
            if (/^\u200b/.test(sufix)) {
              startNode.text(
                text.substring(0, startOffset) + sufix.substring(1)
              );
    ...
    this.onContainer("beforeinput", (event: InputEvent) => {
      ...
      if (!range.collapsed) {
        if (range.commonAncestorNode.attributes(CARD_ELEMENT_KEY) === "body")
      ...
      if (inputType) {
        commandTypes.forEach((type) => {
          if (inputType.indexOf(type) === 0) {
            event.preventDefault();
            const commandName = inputType.substring(type.length).toLowerCase();
  ...
  addEvent(
    ...
    if (!this.globalEvents[type].find((event) => event.type === eventType)) {
