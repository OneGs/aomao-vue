      ...
      (node) => {
        ...
        if (node.isText()) {
          ...
          if (/\u200b/.test(text)) {
            ...
            if (parent && nodeApi.isMark(parent, this.schema)) isRemove = false;
            ...
            node.remove();
            if (parent.get<Element>()?.childNodes.length === 0) parent.remove();
            return;
          } else if (/^\n$/.test(text)) {
        ...
        let attributes: { [k: string]: string } | undefined = node.attributes();
        // 删除空 style 属性
        ...
        if (nodeIsBlock && parent?.name === "p") {
          ...
          parentIsBlock = parent ? nodeApi.isBlock(parent, this.schema) : false;
        }
        ...
        if (nodeApi.isList(node) && parent && nodeApi.isList(parent)) {
          ...
          const appendToTemp = () => {
            if ((tempList.get<Node>()?.childNodes.length ?? 0) > 0) {
          ...
          leftList.forEach((childNode) => {
            ...
            if (!child || child.get<Node>()?.childNodes.length === 0) return;
            prev.after(child);
            prev = child;
          });
          rightList.forEach((childNode) => {
            const child = $(childNode);
            if (!child || child.get<Element>()?.childNodes.length === 0) return;
            prev.after(child);
            prev = child;
          });
          parent.remove();
        ...
        if (nodeApi.isList(node) && parent?.name === "li") {
          ...
          children.each((child, index) => {
            ...
            if (
              !node ||
              (nodeApi.isEmptyWithTrim(node) && !nodeApi.isVoid(node))
            ) {
              return;
            }
            ...
            if (isList) {
              const indent = $(leftLast)?.attributes(list.INDENT_KEY) || "0";
              node.attributes(list.INDENT_KEY, indent);
              list.addIndent(node, 1);
        ...
        if (nodeIsBlock && !nodeIsVoid && nodeApi.html(node).trim() === "") {
          // <p></p> to <p><br /></p>
          if (nodeApi.isRootBlock(node, this.schema) || node.name === "li") {
        ...
        if (nodeIsMark) {
          ...
          if (markPlugin) {
            ...
            if (plugin) {
              if (plugin.node.get<Node>()?.childNodes.length === 1) {
        ...
        if (
          ...
          const cMarkPlugin =
            currentMarkPlugins[currentMarkPlugins.length - 1]?.plugin;
          if (
            pMarkPlugin &&
            cMarkPlugin &&
      ...
      if (node.isText()) {
        ...
        if (/^(\r|\n)+$/.test(text)) {
          ...
          if (
            ...
            (prev && nodeApi.isBlock(prev) && next && nodeApi.isBlock(next)) ||
            node.parent()?.name === "p"
          )
            node.remove();
        }
