...
class CardModel implements CardModelInterface {
  ...
  constructor(editor: EditorInterface, lazyRender = true) {
    this.classes = {};
    this.components = [];
    this.editor = editor;
      ...
      components.forEach(async (card) => {
        ...
        if (
          ...
          (editor.root.inViewport(card.root, true) &&
            editor.trigger("card:async-render-component", card) !== false)
        ) {
          this.asyncComponents.splice(
            this.asyncComponents.findIndex((component) => component === card),
            1
          );
          if (card.root.length > 0 && card.loading) {
            ...
            if (isEngine(editor) && editor.readonly && !editor.ot.isRemote) {
              if (!editor.ot.doc?.type) {
                editor.ot.doc?.create(toJSON0(editor.container));
              } else {
  ...
  replaceNode(node: NodeInterface, name: string, value?: CardValue) {
    ...
    const cardNode = transformCustomTags(
      ...
      } type="${type}" name="${name}" value="${encodeCardValue(value)}"></card>`
    );
      ...
      if (card) {
        ...
        if (!isCurrentActiveCard) {
          if (
            ...
            (trigger !== CardActiveTrigger.CLICK || isEngine(editor))
          ) {
  ...
  focus(card: CardInterface, toStart = false) {
    const editor = this.editor;
    if (!isEngine(editor)) return;
  ...
  remove(selector: NodeInterface | Node | string, hasModify = true) {
    const editor = this.editor;
    if (!isEngine(editor)) return;
  ...
  render(
    ...
    allCards.forEach((cardNode) => {
      ...
      childCards.forEach((childCard) => {
        ...
        const index = readyCards.findIndex((node) => node.equal(childCard));
        if (index > -1) {
  ...
  renderComponent(card: CardInterface, ...args: any) {
    ...
    if (card.contenteditable.length > 0) {
      ...
      center.find(card.contenteditable.join(",")).each((node) => {
        const child = $(node);
        if (child.attributes(DATA_CONTENTEDITABLE_KEY) !== contenteditable)
