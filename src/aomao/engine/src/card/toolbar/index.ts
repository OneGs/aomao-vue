...
export const isCardToolbarItemOptions = (
  ...
  return (
    ["button", "input", "dropdown", "node", "switch"].indexOf(item.type) === -1
  );
};
    ...
    if (items.length > 0) {
      ...
      if (dnd && !isMobile && dnd.type === "dnd") {
        ...
        const dndNode = this.createDnd(
          dnd.content || '<span class="data-icon data-icon-drag"></span>',
        ...
        position.bind(
          ...
          (rect) => {
            if (
              ...
              position.setOffset([this.offset[2], this.offset[3]]);
