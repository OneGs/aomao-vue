...
class Schema implements SchemaInterface {
  ...
  add(rules: SchemaRule | SchemaGlobal | Array<SchemaRule | SchemaGlobal>) {
      ...
      if (isSchemaRule(rule)) {
        ...
        if (rule.attributes) {
          Object.keys(rule.attributes).forEach((key) => {
            ...
            if (key === "style") {
              Object.keys(rule.attributes!.style).forEach((styleName) => {
                if (
                  this.data.globals[rule.type][key] &&
                  this.data.globals[rule.type][key][styleName] ===
              ...
              this.data.globals[rule.type][key] === rule.attributes![key]
            ) {
    ...
    const getCount = (rule: SchemaRule) => {
      ...
      Object.keys(aAttributes).forEach((attributesName) => {
        ...
        if (isSchemaValueObject(attributesValue) && attributesValue.required)
          aCount++;
      });
  ...
  find(callback: (rule: SchemaRule) => boolean): Array<SchemaRule> {
    ...
    SCHEMA_KEYS.forEach((key) => {
      if (key !== "globals") {
        const rules = (this.data[key] as Array<SchemaRule>).filter(callback);
        schemas.push(...rules);
      }
    });
    return schemas;
  ...
  getType(node: NodeInterface | Node, filter?: (rule: SchemaRule) => boolean) {
   ...
   * @returns
   */
  getRule(node: NodeInterface | Node, filter?: (rule: SchemaRule) => boolean) {
    const element = (isNode(node) ? node : node[0]) as Element;
    ...
    if (typeof rule === "object" && typeof rule.test === "function") {
      if (attributesName === "class") {
        return (attributesValue || "")
          .split(/\s+/)
          .every((value) =>
            value.trim() === "" ? true : (rule as RegExp).test(value.trim())
          );
      }
  ...
  addAllowIn(parent: string, child = "p") {
