...
class Scrollbar extends EventEmitter2 {
  ...
  refresh = () => {
    ...
    if (element) {
      ...
      this.yHeight = Math.floor((this.oHeight * this.oHeight) / scrollHeight);
      if (this.x) {
        this.slideX?.css("width", this.xWidth + "px");
        const display =
          Math.round(this.oWidth) - sPLeft - sPRight === this.sWidth ||
          (contentElement &&
      ...
      if (this.y) {
        ...
        const display =
          Math.round(this.oHeight) - sPTop - sPBottom === this.sHeight ||
          (contentElement &&
      ...
      if (
        this.y &&
        contentElement &&
        element.scrollHeight - sPTop - sPBottom !== this.#content!.height()
      ) {
        this.#isScrolling = true;
        element.scrollTop -=
          element.scrollHeight - sPTop - sPBottom - this.#content!.height();
  ...
  wheelXScroll = throttle(
    (event: any) => {
      ...
      const containerWidth = this.#scroll?.getOffsetWidth
        ...
        : width;
      const step = Math.max(
        containerWidth / (isMacos ? 20 - Math.abs(event.wheelDelta) : 8),
        20
      );
  ...
  wheelYScroll = throttle(
    (event: any) => {
      ...
      const step = Math.max(
        containerHeight / (isMacos ? 20 - Math.abs(event.wheelDelta) : 8),
        20
      );
  ...
  destroy() {
    this.slideX?.off(isMobile ? "touchstart" : "mousedown", this.scrollXStart);
    this.slideY?.off(isMobile ? "touchstart" : "mousedown", this.scrollYStart);
    if (isMobile) {
