...
class Selection implements SelectionInterface {
  ...
  constructor(editor: EditorInterface, range: RangeInterface, key = "") {
    this.editor = editor;
    this.range = range;
    this.key = key;
  }
  ...
  move() {
    ...
    if (this.key) {
      ...
      const root = commonAncestorNode.closest(ROOT_SELECTOR);
      if (!this.focus.inEditor() || !this.focus.get<Element>()?.isConnected) {
        this.focus = root.find(
          `[data-${this.focus.attributes(DATA_ELEMENT)}-id="${this.key}"]`
        );
      }
      if (!this.anchor.inEditor() || !this.anchor.get<Element>()?.isConnected) {
        this.anchor = root.find(
          `[data-${this.anchor.attributes(DATA_ELEMENT)}-id="${this.key}"]`
        );
      }
    }
    ...
    if (parent) {
      ...
      this.focus.remove();
      parent[0].normalize();
      if (parent.name === "p" && parent.get<Node>()?.childNodes.length === 0) {
        parent.append($("<br />"));
      }
      if (isSafari) {
  ...
  getNode(
    ...
    if (position === "left" || position === "center") {
      const selectionNode = position !== "center" ? this.anchor : this.focus;
      let focus: NodeInterface | undefined = $(
        this.key
          ? `[data-${selectionNode.attributes(DATA_ELEMENT)}-id="${this.key}"]`
    ...
    if (position === "right" || position === "center") {
      const selectionNode = position !== "center" ? this.focus : this.anchor;
