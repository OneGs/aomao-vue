...
class List implements ListModelInterface {
  ...
  getPluginNameByNodes(blocks: Array<NodeInterface>) {
    ...
    for (let i = 0; i < blocks.length; i++) {
      ...
      switch (block.name) {
        ...
        default:
          //如果节点内包含了列表节点，则跳过此节点
          if (this.editor.node.isBlock(block) && block.find("ul,ol").length > 0)
            break;
          else return "";
      }
  ...
  addStart(block?: NodeInterface) {
    ...
    const startIndent = parseInt(block.attributes(this.INDENT_KEY), 10) || 0;
    ...
    while (prevNode && node.isList(prevNode)) {
      if (prevNode.name === "ol") {
        ...
        const prevStart = parseInt(prevNode.attributes("start"), 10) || 1;
      ...
      if (node.isList(nextNode)) {
        ...
        if (nextNode.name === "ol") {
          ...
          if (nextIndent > 0) {
            ...
            if (currentStart > 1) nextNode.attributes("start", currentStart);
  ...
  insert(fragment: DocumentFragment, range?: RangeInterface) {
    ...
    const { startNode, startOffset } = cloneRange;
    ...
    if (startIsMerge) {
      ...
      if (beforeELement?.name === "li") {
        ...
        beforeChildren?.each((child, index) => {
          if (child.nodeName === "BR") beforeChildren.eq(index)?.remove();
        });
        // 自定义列表，删除第一个卡片
      ...
      if (
        ...
        if (beforeELement.parent()?.get<Node>()?.childNodes.length === 0)
    ...
    const startListElment = safeRange.startNode.closest("li").parent();
    if (!startListElment) {
      apply(safeRange);
      return;
    }
      ...
      if (node.isList(childElement)) {
        ...
        if (node.isCustomize(startListElment)) {
          ...
          if (firstCard && firstCard.isCard()) {
            const cardName =
              firstCard.attributes(CARD_KEY) || firstCard.attributes("name");
            const customizeList = this.toCustomize(childElement, cardName);
            if (customizeList) {
          ...
          (Array.isArray(listElements) ? listElements : [listElements]).forEach(
            (child) => {
              if (editor.node.isList(child)) startListElment?.before(child);
            }
          );
        }
      }
    ...
    if (lasetELement) {
      ...
      if (endNode.name === "br") endNode.remove();
        ...
        endChildren.each((child, index) => {
          if (child.nodeName === "BR") endChildren.eq(index)?.remove();
        });
        if (node.isCustomize(lasetELement)) {
          ...
          lasetELement
            .first()
            ?.after(node.isBlock(endNode) ? endNode.children() : endNode);
        } else {
  ...
  blockToItem(
    ...
    const globalStyles = globals.style || {};
    const styles = block.css();
    if (styles["text-align"]) this.addAlign(item, styles["text-align"] as any);
    delete styles["text-align"];
  ...
  toCustomize(
    ...
    if (Array.isArray(blocks)) {
      ...
      blocks.forEach((block) => {
        ...
        nodes = nodes.concat(this.toCustomize(block, cardName, value, tagName));
      });
      return nodes;
    } else {
      const customizeRoot = $(
