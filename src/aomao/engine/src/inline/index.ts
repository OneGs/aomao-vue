...
class Inline implements InlineModelInterface {
  ...
  unwrap(range?: RangeInterface | NodeInterface) {
    ...
    const safeRange =
      !range || !isRangeInterface(range) ? change.range.toTrusty() : range;
    this.repairRange(safeRange);
    mark.split(safeRange);
    const inlineNodes =
      range && !isRangeInterface(range) ? [range] : this.findInlines(safeRange);
    // 清除 Inline
  ...
  splitOnCollapsed(range: RangeInterface, keelpNode?: NodeInterface | Node) {
    ...
    if (
      ...
      (node.isInline(startNode) || (startParent && node.isInline(startParent)))
    ) {
      // 获取上面第一个非inline标签
  ...
  splitOnExpanded(range: RangeInterface) {
    ...
    if (
      ...
      const endIsInline =
        node.isInline(endNode) || (endParent && node.isInline(endParent));
    ...
    const handleRange = (
      ...
      if (toStart) {
        ...
        const startOffsetNode =
          startChildren.eq(startOffset) || startChildren.eq(startOffset - 1);
        if (
          !allowBlock &&
          ...
          (startNode.type !== Node.ELEMENT_NODE ||
            (!!startOffsetNode && !nodeApi.isBlock(startOffsetNode)))
        )
          return;
        cloneRange.select(startParent, true);
        ...
        const startOffsetNode = startNodeCloneChildren.eq(startOffsetClone);
  ...
  repairCursor(node: NodeInterface | Node) {
    ...
    if (
      ...
      node.closest(CARD_SELECTOR).attributes(CARD_EDITABLE_KEY) === "false" ||
      nodeApi.isVoid(node) ||
      node.isCard()
    ) {
  ...
  flat(node: NodeInterface | RangeInterface, schema?: SchemaInterface) {
    ...
    if (nodeApi.isInline(node, schema) && node.name !== "br") {
      ...
      //不允许放入mark
        ...
        let parentMark: NodeInterface | undefined = markApi.closest(node);
