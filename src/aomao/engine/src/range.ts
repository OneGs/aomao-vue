...
class Range implements RangeInterface {
   ...
   */
  enlargeToElementNode = (toBlock = false, toTop = true) => {
   ...
   * @param key 唯一标识
   */
  createSelection = (key = ""): SelectionInterface => {
   ...
   */
  getSubRanges = (includeCard = false, filterSingleSelectableCard = true) => {
      ...
      if (child.isText()) {
        ...
        if (
          ...
          (filterSingleSelectableCard &&
            (cardComponent.constructor as CardEntry).singleSelectable === false)
        )
          return;
        ...
        if (start < 0) {
          if (end < 0) return;
          if (end === 0) {
            docRange.setOffset(childNode, this.startOffset, offset + 1);
          } else {
            docRange.setOffset(childNode, this.startOffset, this.endOffset);
          }
        } else {
  ...
  filterPath(includeCardCursor = false) {
    ...
    return (node: Node) => {
      ...
      if (includeCardCursor && element.isCard() && cardCaches.includes(element))
        return true;
