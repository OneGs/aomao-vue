...
class RangeColoring implements RangeColoringInterface {
  ...
  drawBackground(
    ...
    const parentWidth =
      containerElement.get<Element>()?.clientWidth || containerElement.width();
      ...
      if (result) {
        if (Array.isArray(result)) subRanges = result;
          ...
          if (result.x < 0) {
            ...
            child.css("transform", `translateX(${result.x}px) translateY(0)`);
            result.x = 0;
          }
  ...
  drawCard(node: NodeInterface, cursor: NodeInterface, member: Member) {
    ...
    if (nodeRect.height === 0) {
      ...
      mask.get<HTMLElement>()!.style.cssText =
        cssText + `height: ${nodeRect.height}px; width: ${nodeRect.width}px;`;
    }
  ...
  remove(uuid: string) {
    (this.engine.scrollNode?.get<Element>() ?? this.root.get<Element>())
      ...
      .forEach((dataElement) => {
        ...
        (dataElement.parentElement ?? dataElement.parentNode)?.removeChild(
          dataElement
        );
      });
    this.engine.card.each((component) => {
      if (component.isEditable || component.selectedByOther !== uuid) return;
      // 取消锁定
