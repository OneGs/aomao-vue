...
class Consumer implements ConsumerInterface {
  ...
  insertText(root: NodeInterface, path: Path, offset: number, text: string) {
    ...
    if (
      ...
      if (endNode.firstChild?.nodeName === "BR") endNode.firstChild.remove();
  ...
  handleOperation(op: TargetOp, isRemote?: boolean) {
    ...
    if (path.length !== 0) {
      ...
      if ("id" in op && op.id && op.bi && op.bi > -1) {
        const target = this.engine.container.find(`[${DATA_ID}="${op.id}"]`);
      ...
      if (start && end) {
        ...
        try {
          ...
          if (!range.collapsed) {
            ...
            if (startCard && endCard && startCard?.root.equal(endCard.root)) {
              let startEditableElement =
                range.startNode.closest(EDITABLE_SELECTOR);
              if (startEditableElement.length === 0)
                startEditableElement = range.startNode.find(EDITABLE_SELECTOR);
              let endEditableElement = range.endNode.closest(EDITABLE_SELECTOR);
              if (endEditableElement.length === 0)
                endEditableElement = range.endNode.find(EDITABLE_SELECTOR);
              if (
