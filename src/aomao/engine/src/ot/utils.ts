...
export const isTransientElement = (
  ...
  if (element.nodeType === Node.ELEMENT_NODE) {
    ...
    if (transientElements) {
      ...
      const element = transientElements.find((element) => element === node[0]);
      if (element) {
        if (element["__card_root"]) loadingCards?.push(element["__card_root"]);
        return true;
      }
    }
...
export const isTransientAttribute = (
  ...
  if (
    isCard(element) &&
    ["id", "class", "style", CARD_LOADING_KEY, CARD_EDITABLE_KEY].includes(attr)
  )
    return true;
...
export const isReverseOp = (op: Op, next: Op) => {
  ...
  if (deleteStringOp.sd && insertStringNext.si) {
    return (
      isEqual(deleteStringOp.sd, insertStringNext.si) && isEqual(op.p, next.p)
    );
  }
...
const isReversePath = (op: Path, next: Path, length = 1): boolean => {
...
export const toDOM = (ops: Op[] | Op[][]): Node => {
  ...
  for (; i < ops.length; i++) {
    if (Array.isArray(ops[i])) {
      ...
      if (elementName) {
        element = document.createElement(elementName) as HTMLElement | null;
        for (const attr in ops[i]) {
...
export const findFromDoc = (
  ...
  for (let i = 1; i < data.length; i++) {
    if (i === 1) {
      ...
      if (typeof attributes === "object" && callback && callback(attributes)) {
        return {
          path: [],
          name: data[0],
          attributes,
