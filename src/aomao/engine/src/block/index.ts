...
class Block implements BlockModelInterface {
  ...
  findPlugin(block: NodeInterface): BlockInterface | undefined {
    ...
    const result: BlockInterface | undefined = this.pluginCaches.get(key);
  ...
  findTop(parentNode: NodeInterface, childNode: NodeInterface) {
    ...
    if (schema.isAllowIn(parentNode.name, childNode.name)) return parentNode;
  ...
  split(range?: RangeInterface) {
    ...
    const activeMarks = mark.findMarks(cloneRange).filter((mark) => {
      ...
      return plugin?.copyOnEnter !== false && plugin?.followStyle !== false;
    });
  ...
  insert(
    ...
    let rightNodes = selection.getNode(container, "right", true, (child) => {
      if (child.isCard()) {
  ...
  setBlocks(block: string | { [k: string]: any }, range?: RangeInterface) {
    ...
    const targetPlugin = targetNode ? this.findPlugin(targetNode) : undefined;
    const selection = safeRange.createSelection();
    blocks.forEach((child) => {
      // Card 不做处理
      ...
      while (nextNode && tags.indexOf(nextNode.name) > 0) {
        ...
        if (
          ...
          nextAttributes["class"] ===
            (prevAttributes ? prevAttributes["class"] : undefined) &&
  ...
  insertEmptyBlock(range: RangeInterface, block: NodeInterface) {
    ...
    marks.forEach((mark) => {
      ...
      //插件判断
      if (plugin?.copyOnEnter !== false && plugin?.followStyle !== false) {
        mark = nodeApi.clone(mark, false, false);
        node.before(mark);
        mark.append(node);
        node = mark;
