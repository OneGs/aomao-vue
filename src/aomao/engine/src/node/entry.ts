...
class NodeEntry implements NodeInterface {
  ...
  isEditableCard() {
    const element = this.get<HTMLElement>();
    return element?.nodeType === Node.ELEMENT_NODE && isEditableCard(element);
  }
  ...
  inEditor(root?: Node | NodeInterface) {
    ...
    return !!element && inEditor(element, root ? root[0] ?? root : undefined);
  }
  ...
  get<E extends Node>(index = 0): E | null {
  ...
  prevElement(): NodeInterface | null {
    const node =
      this.length === 0 ? null : this.get<Element>()!.previousElementSibling;
    return node ? new NodeEntry(node) : null;
  }
  ...
  off(
    ...
    this.each((node, i) => {
      ...
      if (this.events[i]) this.events[i].off(eventType, listener, options);
    });
    return this;
  }
  ...
  traverse(
    callback: (node: NodeInterface) => boolean | void | null | NodeInterface,
    order = true,
    ...
    const walk = (node: NodeInterface) => {
      ...
      if (
        ...
        (!includeCard || (includeCard === "editable" && !node.isEditableCard()))
      ) {
        return;
      }
  ...
  inViewport(view: NodeInterface, simpleMode = false) {
    let viewNode = null;
    ...
    if (viewNode)
      (viewNode.parentElement ?? viewNode.parentNode)?.removeChild(viewNode);
    // 简单模式，只判断任一方向是否在视口内
    if (simpleMode) {
      return (
        (top > 0 && top <= vp.bottom) || (bottom > 0 && bottom <= vp.bottom)
      );
    }
    return (
      top > 0 &&
  ...
  scrollIntoView(
    ...
    if (typeof view.document?.body.scrollIntoView === "function") {
      ...
      if (viewElement)
        (viewElement.parentElement ?? viewElement.parentNode)?.removeChild(
          viewElement
        );
    }
  }
