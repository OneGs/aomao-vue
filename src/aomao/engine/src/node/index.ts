...
class NodeModel implements NodeModelInterface {
  ...
  isVoid(
    ...
    return (
      schema.find((rule) => rule.name === name && rule.isVoid === true).length >
      0
    );
  }
  ...
  isInline(
    ...
    return (
      schema
        .getTags("inlines")
        .includes(
          (node as Node).nodeName?.toLowerCase() ?? (node as NodeInterface).name
  ...
  isEmpty(node: NodeInterface, withTrim?: boolean) {
    if (node.length > 0 && node.isElement()) {
      ...
      if (
        ...
        (results.some(
          (e) => e.hasAttribute(CARD_KEY) || e.hasAttribute(READY_CARD_KEY)
        ) &&
          !results.some((e) => e.getAttribute(DATA_ELEMENT) === EDITABLE))
      ) {
        return false;
      }
      // 非br节点的空节点不为空
   ...
   */
  wrap(source: NodeInterface | Node, outer: NodeInterface, mergeSame = false) {
    ...
    if (mergeSame && this.isMark(outer)) {
      ...
      if (source.name === outer.name) {
        ...
        Object.keys(attributes).forEach((key) => {
          if (!outer.attributes(key)) outer.attributes(key, attributes[key]);
          else {
   ...
   */
  merge(source: NodeInterface, target: NodeInterface, remove = true) {
    ...
    if (this.isCustomize(source)) {
      ...
      if (!sourceFirst?.isCard()) {
        ...
        const plugin = plugins.find(
          (p) => (p.constructor as PluginEntry).pluginName === pluginName
        );
        if (plugin?.cardName) {
      ...
      if (this.isCustomize(target) && !sourceFirst?.equal(target)) {
        ...
        if (
          targetFirst?.isCard() &&
          sourceFirst!.attributes(CARD_KEY) === targetFirst.attributes(CARD_KEY)
        )
          targetFirst.remove();
      }
    }
    ...
    while (child) {
      ...
      // 孤立的零宽字符删除
        ...
        if (
          !parent ||
          (!this.isMark(parent) &&
            ((prev && !this.isInline(prev)) || (next && !this.isInline(next))))
        ) {
          child.remove();
          child = next;
          continue;
   ...
   * @param target 新节点
   */
  replace(source: NodeInterface, target: NodeInterface, copyId = false) {
  ...
  insert(
    ...
    if (this.isBlock(node)) {
      ...
      if (
        ...
        splitNode = removeCurrentEmptyBlock ? commonAncestorNode : undefined;
      } else {
        let isFirst = false;
      ...
      let blockNode = block.closest(
        range.startNode.isEditable()
          ? range.cloneRange().shrinkToElementNode().shrinkToTextNode()
              .startNode
          : range.startNode
      );
      if (blockNode.isRoot() && !range.startNode.next()) {
        ...
        while (
          ...
          !schema.isAllowIn(parentBlock.name, node.nodeName.toLowerCase())
        ) {
    ...
    return range
      .select(
        node,
        this.isVoid(node) || node.nodeType === Node.TEXT_NODE ? false : true
      )
      .shrinkToElementNode()
      .collapse(false);
  }
  ...
  mergeChild(node: NodeInterface) {
    ...
    while (childDom) {
      ...
      while (
        ...
        ((topTags.indexOf(childDom.name) > -1 && !this.isList(childDom)) ||
   ...
   */
  removeSide(node: NodeInterface, tagName = "br") {
    // 删除第一个 BR
  ...
  html(node: NodeInterface, val?: string): NodeInterface | string {
    if (val === undefined) {
      return node.length > 0 ? node.get<HTMLElement>()?.innerHTML || "" : "";
    }
   ...
   * @return 复制后的元素节点
   */
  clone(node: NodeInterface, deep?: boolean, copyId = true): NodeInterface {
