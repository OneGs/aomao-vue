...
class Parser implements ParserInterface {
    ...
    while (value) {
      ...
      if (node.isCard()) {
        ...
        if (nodeApi.isBlock(newNode, schema)) {
          //排除之前的过滤规则后再次过滤
          value = conversion.transform(newNode, (r) => oldRules.indexOf(r) < 0);
          continue;
        }
      }
      //排除之前的过滤规则后再次过滤
  ...
  normalize(
    ...
    // 过滤分割
    const filter = (node: NodeInterface, type = "mark") => {
      //获取节点属性样式
      ...
      if (
        ((filterAttrCount === attrCount && filterStyleCount === styleCount) ||
      ...
      if (node.isElement()) {
        ...
        if (nodeApi.isMark(node, schema)) {
          ...
          if (rule) {
            ...
            if (!type) {
              if (conversion) {
                const newChildren = this.convert(conversion, newNode, schema);
                if (newChildren && newChildren.length > 0) {
                  ...
                  node.append(
                    newNode.length === 0 ? newChildren : newNode.children()
                  );
                  return;
                }
              }
            ...
            while (type === "mark") {
              ...
              if (!type) {
                if (conversion) {
                  ...
                  if (newChildren && newChildren.length > 0) {
                    newNode =
                      newNode.length > 0 ? newNode.children() : newChildren;
                    type = "mark";
                    continue;
                  }
                }
    ...
    while (child) {
      ...
      if (child.isElement()) {
        ...
        if (schema && attributes[DATA_ELEMENT] !== EDITABLE) {
          ...
          if (type === undefined) {
            ...
            if (
              ...
              // 子节点只有一个
              parent.get<HTMLElement>()?.childNodes.length === 1 &&
              // 没有子节点
        ...
        if (
          ...
          const result = callbacks.onOpen(child, name, attributes, styles);
          //终止遍历当前节点
          if (result === false) {
            child = child.next();
            continue;
  ...
  toValue(
    ...
    this.traverse(root, schema, conversion, {
      onOpen: (child, name, attributes, styles) => {
        if (
          editor.trigger("parse:value", child, attributes, styles, result) ===
          false
        )
          return false;
    ...
    this.traverse(
      ...
      null,
      {
        onOpen: (node, name, attributes, styles) => {
          if (
            editor.trigger("parse:text", node, attributes, styles, result) ===
            false
          ) {
            return false;
          }
        ...
        onClose: (node, name) => {
          ...
          if (name === "p" || nodeApi.isBlock(node, schema || editor.schema)) {
            const children = Array.from(node.get<HTMLElement>()!.childNodes);
            // 子节点还有block节点，则不换行
            if (
              children.length === 0 ||
              children.some((child) => {
                ...
                const type = (schema || editor.schema).getType(child);
