    ...
    const rule = this.data.find((item) => {
      ...
      if (typeof from === "string") {
        ...
        result =
          ...
          elementNames.some((elementName: string) => {
            ...
            return (
              Object.keys(elementRules.style || {}).every((styleName) => {
                const styleValue = elementRules.style![styleName];
                return !!style[styleName] && Array.isArray(styleValue)
                  ? styleValue.indexOf(style[styleName]) > -1
                  : styleValue === style[styleName];
              }) &&
              Object.keys(elementRules.attributes || {}).every(
                (attributesName) => {
                  const attributesValue =
                    elementRules.attributes![attributesName];
                  return !!style[attributesName] &&
                    Array.isArray(attributesValue)
                    ? attributesValue.indexOf(style[attributesName]) > -1
                    : attributesValue === style[attributesName];
                }
              )
            );
          });
      ...
      if (result) {
        if (typeof to === "string") {
          ...
          const result =
            typeof to === "function" ? to(name, style, attributes) : to;
