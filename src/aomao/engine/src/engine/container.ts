...
class Container {
  ...
  _init() {
    ...
    if (className !== undefined) {
      (Array.isArray(className) ? className : className.split(/\s+/)).forEach(
        (name) => {
  ...
  handleClick = (event: MouseEvent) => {
    ...
    if (event.target && !engine.readonly && isEngine(engine)) {
      ...
      if (
        autoPrepend !== false &&
        firstBlock &&
        event.offsetY < (firstBlock.get<HTMLElement>()?.offsetTop || 0) &&
  ...
  handleFocus = () => {
    const { engine } = this.options;
    this.triggerFoucs();
    if (!engine.ot.isStopped() && engine.isEmpty()) engine.change.initValue();
  };
