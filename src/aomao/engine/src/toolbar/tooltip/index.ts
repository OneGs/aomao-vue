...
class Tooltip {
  static show(
    ...
    // 设置提示文字
    if (typeof title === "string") root.find("[data-role=tooltip]").html(title);
    else root.find("[data-role=tooltip]").append(title);
    // 计算定位
    const body = $(document.body);
    body.append(root);
    const rect = domAlign(root.get<HTMLElement>(), node.get<HTMLElement>(), {
    ...
    if (align !== options.placement) {
      root
        .removeClass(`data-tooltip-placement-${options.placement}`)
