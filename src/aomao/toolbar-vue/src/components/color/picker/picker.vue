<template>
  <div class="colorpicker-board" @mousedown="triggerMouseDown">
    <div
...
export default defineComponent({
  ...
  setup(props, cxt) {
    const activeColors = (
      values: Array<Array<string | { value: string; active: boolean }>>,
      activeValue: string
    ) => {
    ...
    const colors = ref(
      activeColors(
        props.colors || Palette.getColors(),
    ...
    const text = props.engine.language.get(
      "toolbar",
      "colorPicker",
      props.defaultColor === "transparent" ? "nonFillText" : "defaultText"
    );
    return {
      colors,
      triggerSelect,
