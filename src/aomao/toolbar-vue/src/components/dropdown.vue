    ...
    const update = (values?: string | number | string[]) => {
      ...
      const defaultItem =
        props.items.length > 0
          ? props.items.find((item) => item.isDefault === true) ||
            props.items[0]
          : null;
      ...
      if (item) {
        if (cxt.slots.default) {
          ...
          buttonContent.value = {
            icon: props.icon,
            content: props.content(props.engine),
          };
        ...
        buttonContent.value = {
          ...
          content:
            typeof defaultItem.content === "function"
              ? defaultItem.content(props.engine)
              : defaultItem.content,
        };
      }
      valuesVar.value =
...
.editor-toolbar-popup
  .toolbar-dropdown
  .toolbar-dropdown-list:not(.toolbar-dropdown-placement-bottom) {
  bottom: 32px;
  top: auto;
  overflow: auto;
}
