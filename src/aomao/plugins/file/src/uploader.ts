...
import { RequestData, RequestHeaders } from "@/aomao/engine/src";
    ...
    request.upload(
      {
        ...
        onReady: (fileInfo) => {
          ...
          const component = card.insert<FileValue, FileComponent<FileValue>>(
            "file",
            {
              status: "uploading",
              name: fileInfo.name,
        ...
        onSuccess: (response, file) => {
          ...
          const preview =
            response.preview || (response.data && response.data.preview);
          const download =
        ...
        onError: (error, file) => {
          ...
          card.update<FileValue>(component.id, {
            status: "error",
            message:
              error.message || language.get<string>("file", "uploadError"),
          });
          delete this.cardComponents[file.uid || ""];
        },
      },
