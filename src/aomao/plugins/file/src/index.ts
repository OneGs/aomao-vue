...
export default class<T extends FileOptions = FileOptions> extends Plugin<T> {
  ...
  pasteHtml = (node: NodeInterface) => {
    ...
    if (node.isElement()) {
      ...
      if (type === FileComponent.cardName) {
        ...
        editor.card.replaceNode(node, FileComponent.cardName, cardValue);
        node.remove();
        return false;
      }
    }
  ...
  parseHtml = (
    ...
    root
      ...
      .each((cardNode) => {
        const node = $(cardNode);
        const card = this.editor.card.find(node) as FileComponent<FileValue>;
        const value =
        ...
        if (value?.url && value.status === "done") {
          ...
          }" data-value="${encodeCardValue(value)}"><a target="_blank" href="${
            value.url
