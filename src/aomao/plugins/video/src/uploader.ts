...
import type { RequestData, RequestHeaders } from "@/aomao/engine/src";
  ...
  async execute(files?: Array<File> | MouseEvent | string, ...args: any) {
    ...
    if (!Array.isArray(files)) {
      ...
      files = await request.getFiles({
        event: files,
        accept: isAndroid ? "video/*" : accepts.length > 0 ? accepts : "",
        multiple,
      });
    }
    ...
    request.upload(
      {
        ...
        onReady: (fileInfo) => {
          ...
          const component = card.insert<VideoValue, VideoComponent<VideoValue>>(
            "video",
            {
              status: "uploading",
              name: fileInfo.name,
        ...
        onSuccess: (response, file) => {
          ...
          const width: number =
            response.width || (response.data && response.data.width);
          const height: number =
            response.height || (response.data && response.data.height);
        ...
        onError: (error, file) => {
          ...
          card.update<VideoValue>(component.id, {
            status: "error",
            message:
              error.message || language.get<string>("video", "uploadError"),
          });
          delete this.cardComponents[file.uid || ""];
        },
      },
      ...
      success: (response: any) => {
        ...
        if (!result) {
          ...
          if (result.result === false) {
            failed(result.data || language.get("video", "loadError"));
          } else
            success({
              ...result.data,
              status:
