...
class VideoComponent<T extends VideoValue = VideoValue> extends Card<T> {
  ...
  onBeforeRender = (action: "query" | "download" | "cover", url: string) => {
    const videoPlugin = this.editor.plugin.findPlugin<VideoOptions>("video");
    if (videoPlugin) {
  ...
  initPlayer() {
    ...
    video.onloadedmetadata = () => {
      ...
      if (
        fullEditor &&
        this._fullWidth &&
        (value.naturalWidth < this._fullWidth || typeof fullEditor === "number")
      ) {
  ...
  toolbar() {
    ...
    const options = editor.plugin.findPlugin<VideoOptions>("video")?.options;
  ...
  onSelectByOther(
    ...
    this.container?.css("outline", selected ? "2px solid " + value!.color : "");
  ...
  handleClick = (event: MouseEvent) => {
    ...
    if (isEngine(editor) && !this.activated) {
      editor.card.activate(this.root, CardActiveTrigger.MOUSE_DOWN, event);
    }
  };
