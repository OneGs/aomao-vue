...
class EmbedComponent<V extends EmbedValue = EmbedValue> extends Card<V> {
  ...
  toolbar() {
          ...
          items.push(
            {
              ...
              title: editor.language.get<string>("embed", "expand"),
              onClick: () => this.expand(),
            },
            {
              key: "collapse",
              type: "button",
              content: '<span class="data-icon data-icon-compact-display" />',
              title: editor.language.get<string>("embed", "collapse"),
    ...
    const options = editor.plugin.findPlugin<EmbedOptions>("embed")?.options;
