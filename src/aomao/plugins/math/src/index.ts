...
> extends Plugin<T> {
  ...
  query(
    ...
    this.#request[key] = request.ajax({
      ...
      success: (response) => {
        ...
        if (result.result) {
          const isUrl =
            result.data.indexOf("http") === 0 || result.data.indexOf("/") === 0;
          let url = result.data;
          if (!isUrl) {
            url = this.exConvertToPx(result.data);
            url =
              (url.indexOf("data:") < 0 ? "data:image/svg+xml," : "") +
  ...
  exConvertToPx(svg: string) {
    ...
    if (exWidth) {
      const pxWidth = parseFloat(exWidth.substring(0, exWidth.length - 2)) * 9;
  ...
  pasteHtml = (node: NodeInterface) => {
    ...
    if (node.isElement()) {
      ...
      if (type && type === MathComponent.cardName) {
        ...
        editor.card.replaceNode(node, MathComponent.cardName, cardValue);
        node.remove();
        return false;
      }
    }
  ...
  parseHtml = (
    ...
    root
      ...
      .each((cardNode) => {
        const node = $(cardNode);
        const card = this.editor.card.find<MathValue, MathComponent<MathValue>>(
          node
        );
        const value =
