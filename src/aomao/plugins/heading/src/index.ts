...
> extends BlockPlugin<T> {
  ...
  tagName = ["h1", "h2", "h3", "h4", "h5", "h6"].filter(
    (name) =>
      !this.options.enableTypes || this.options.enableTypes.indexOf(name) > -1
  );
    ...
    container.find(this.tagName.join(",")).each((heading) => {
      ...
      if (id) {
        ...
        button.on("mouseenter", () => {
          Tooltip.show(button, language.get("copyAnchor", "title").toString());
        });
  ...
  showAnchor = () => {
    ...
    if (block.length === 0 || card.closest(block, true) || !editor.isFocus()) {
      return;
    }
    ...
    const rect = block.get<Element>()!.getBoundingClientRect();
    const left = Math.round(
      rect.left - parentRect.left - button.get<Element>()!.clientWidth - 1
    );
    const top = Math.round(
      rect.top -
        parentRect.top +
    ...
    button.on("click", (e) => {
      ...
      if (clipboard.copy(url)) {
        ...
        editor!.messageError("copy", language.get("copy", "error").toString());
      }
    });
  };
  ...
  execute(type: "h1" | "h2" | "h3" | "h4" | "h5" | "h6" | "p") {
    ...
    // 未启用
    if (type !== "p" && enableTypes && enableTypes.indexOf(type) < 0) return;
