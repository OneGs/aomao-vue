  ...
  implements TableInterface<V>
{
  ...
  colMinWidth =
    this.editor.plugin.findPlugin<TableOptions>("table")?.options.colMinWidth ||
    40;
  rowMinHeight =
  ...
  init() {
    ...
    if (isEngine(editor)) {
      ...
      editor.event.listeners["keydown:tab"].unshift((event: KeyboardEvent) => {
  ...
  toolbar(): (ToolbarItemOptions | CardToolbarItemOptions)[] {
    const editor = this.editor;
    const getItems = (): (ToolbarItemOptions | CardToolbarItemOptions)[] => {
      ...
      const funBtns: Array<ToolbarItemOptions | CardToolbarItemOptions> = [
        {
          key: "color",
          type: "node",
          title: editor.language.get<string>("table", "color", "title"),
    ...
    const options = editor.plugin.findPlugin<TableOptions>("table")?.options;
    ...
    const overflowLeftConfig = tableOptions.maxLeftWidth
      ? {
          onScrollX: (x: number) => {
            ...
            this.wrapper?.css("margin-left", `-${x > max ? max : x}px`);
            if (x > 0) {
          ...
          getScrollLeft: (left: number) => {
            return left - removeUnit(this.wrapper?.css("margin-left") || "0");
          },
          getOffsetWidth: (width: number) => {
            return width + removeUnit(this.wrapper?.css("margin-left") || "0");
          },
        }
      : undefined;
  ...
  remoteRefresh() {
    ...
    if (superValue.cols > colCount) {
      colsHeader.append(
        $(this.template.renderColsHeader(superValue.cols - colCount)).find(
    ...
    colElements.forEach((colElement, index) => {
      ...
      colItems
        .eq(index)
        ?.css("width", `${Math.max(parseInt(width), this.colMinWidth)}px`);
    });
    ...
    if (superValue.rows > rowCount) {
      rowsHeader.append(
        $(this.template.renderRowsHeader(superValue.rows - rowCount)).find(
    ...
    rowElements.forEach((rowElement, index) => {
      rowItems
        ...
        ?.css(
          "height",
          Math.max(parseInt(rowElement.css("width")), this.rowMinHeight)
        );
    });
    // this.conltrollBar.refresh();
    this.scrollbar?.refresh();
