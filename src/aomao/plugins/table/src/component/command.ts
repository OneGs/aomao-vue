...
class TableCommand extends EventEmitter2 implements TableCommandInterface {
  ...
  private tableCleared = false;
  private rowCleared = false;
  private colCleared = false;
  ...
  insertColAt(
    ...
    head.css("width", head.get<HTMLElement>()!.offsetWidth + totalWidth + "px");
    ...
    while (counter > 0) {
      ...
      const width = Array.isArray(widths) ? widths[count - counter] : widths;
    ...
    trs.each((tr, r) => {
      ...
      for (let r = 0; r < count; r++) {
        ...
        $(td).attributes(DATA_TRANSIENT_ATTRIBUTES, "table-cell-selection");
        nodeId.generate(td);
      }
    });
  ...
  insertCol(position?: "left" | "end" | "right", count = 1, ...args: any) {
    const { selection, wrapper } = this.table;
    const { tableModel } = selection;
  ...
  removeCol(...args: any) {
    ...
    table.forEach((trModel, r) => {
      for (let _c = selectArea.end.col; _c >= selectArea.begin.col; _c--) {
        ...
        if (helper.isEmptyModelCol(tdModel)) {
          ...
          if (
            ...
            const colRemoved = Math.min(
              count,
              tdModel.parent.col + parentTd.colSpan - selectArea.begin.col
            );
            if (parentTd.element) {
          ...
          if (tdModel.isMulti) {
            ...
            const cutHeader = _c + tdModel.colSpan - 1 > selectArea.end.col;
  ...
  insertColLeft(count = 1) {
    this.insertCol("left", count);
  }

  insertColRight(count = 1) {
  ...
  insertRowAt(index: number, count: number, isUp?: boolean, ...args: any) {
    ...
    const nodeId = this.editor.nodeId;
    const _loop = () => {
      const tr = this.tableRoot?.get<HTMLTableElement>()?.insertRow(insertRow);
      if (!tr) return;
      insertTdProps.forEach((props) => {
        const td = tr.insertCell();
        td.setAttribute(DATA_TRANSIENT_ATTRIBUTES, "table-cell-selection");
        td.innerHTML = this.table.template.getEmptyCell();
        td.colSpan = props.tdBase.colSpan;
      });
      $(baseRowBar)[insertMethod]($((baseRowBar as HTMLElement).outerHTML));
      nodeId.generate(tr);
  ...
  insertRow(position?: "up" | "end" | "down", count = 1, ...args: any) {
      ...
      trModel.forEach((tdModel, c) => {
        if (
          ...
          if (r + tdModel.rowSpan - 1 > end.row) {
            const insertIndex = selection.getCellIndex(end.row + 1, c);
            const td = (trs[end.row + 1] as HTMLTableRowElement).insertCell(
              insertIndex
            );
    ...
    if (element.name === "table") {
      ...
      if (isSingleTd) {
        ...
        if (rowCount + begin.row > tableModel.rows) {
          const insertRowCount = rowCount + begin.row - tableModel.rows;
      ...
      selection.each((tdModel, r, c) => {
        ...
        if (
          ...
          element.rowSpan = Math.min(paste_td.rowSpan, newArea.end.row - r + 1);
          element.colSpan = Math.min(paste_td.colSpan, newArea.end.col - c + 1);
    ...
    selection.each((tdModel, r, c) => {
      ...
      if (tdModel.element) {
        ...
        if (tdModel.element.innerText.trim() !== "") {
          content.unshift(
            $(tdModel.element).find(Template.TABLE_TD_CONTENT_CLASS).html()
          );
        }
  ...
  splitCell(...args: any) {
    ...
    selection.each((cell, row, col) => {
      ...
      for (let r = rows - 1; r >= row; r--) {
        ...
        const tr = r >= trs.length ? this.tableRoot!.find("tr")[r] : trs[r];
        for (let c = cols - 1; c >= col; c--) {
          ...
          if (!helper.isEmptyModelCol(tdModel) && tdModel.isMulti) {
            ...
            const _td2 = (tr as HTMLTableRowElement).insertCell(_insertIndex2);
