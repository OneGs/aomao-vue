...
class ControllBar extends EventEmitter2 implements ControllBarInterface {
  ...
  activeHeader() {
    ...
    if (!allCol && allRow) {
      ...
      const left = colBars?.eq(begin.col)?.get<HTMLElement>()?.offsetLeft || 0;
  ...
  startChangeCol(trigger: NodeInterface, event: MouseEvent | TouchEvent) {
    ...
    this.changeSize = {
      trigger: {
        element: trigger,
        height: removeUnit(getComputedStyle(trigger.get<Element>()!, "height")),
        width: removeUnit(getComputedStyle(trigger.get<Element>()!, "width")),
      },
      element: col,
      ...
      const left =
        this.draggingHeader.index !== colBars.length
          ? colBars.eq(this.draggingHeader.index)!.get<HTMLElement>()!
              .offsetLeft + 2
          : colBars.eq(this.draggingHeader.index - 1)!.get<HTMLElement>()!
              .offsetLeft +
            colBars.eq(this.draggingHeader.index - 1)!.get<HTMLElement>()!
              .offsetWidth +
            2;
      ...
      const top =
        this.draggingHeader.index !== rowBars.length
          ? rowBars.eq(this.draggingHeader.index)!.get<HTMLElement>()!
              .offsetTop + 2
          : rowBars.eq(this.draggingHeader.index - 1)!.get<HTMLElement>()!
              .offsetTop +
            rowBars.eq(this.draggingHeader.index - 1)!.get<HTMLElement>()!
              .offsetHeight -
            2;
  ...
  handleTriggerMenu(menu: NodeInterface) {
    if (!menu.hasClass("disabled")) {
      ...
      if (inputNode.length > 0) {
        args = Math.min(
          parseInt(inputNode.get<HTMLInputElement>()?.value || "1", 10),
          this.MAX_INSERT_NUM
        );
      }
