    ...
    if (cols.length !== 0) {
      ...
      const widths = (function (table) {
        ...
        tds.each((_, index) => {
          ...
          const tdColSpan = ($td[0] as HTMLTableDataCellElement).colSpan;
          if (colWidth) {
    ...
    if (cols.length !== 0) {
      for (let c = cols.length - 1; c >= 0; c--) {
        ...
        if (_width && !_width.endsWith("%")) {
          ...
          if (widthValue !== NaN) cols.eq(c)?.attributes("width", widthValue);
        }
      ...
      const widths = (function (table) {
        ...
        tds.each((_, index) => {
          ...
          const tdColSpan = (tdNode[0] as HTMLTableDataCellElement).colSpan;
          if (colWidth) {
            colWidth = colWidth.split(",");
          } else if (tdWidth) {
            tdWidth = parseInt(tdWidth) / tdColSpan;
