...
class TableSelection extends EventEmitter2 implements TableSelectionInterface {
  ...
  focusCell(cell: NodeInterface | Node, start = false) {
      ...
      if (editableElement.length > 0) {
        ...
        if (block) {
          ...
          if (
            ...
            (event instanceof MouseEvent ? event : event.touches[0]).clientY
          )
            return next();
          const node = $("<p><br /></p>");
      ...
      for (let _c2 = begin.col; _c2 <= end.col; _c2++) {
        ...
        if (!helper.isEmptyModelCol(tdModel) && tdModel.element) {
          ...
          if (tdModel.element.parentElement)
            trHeight = $(tdModel.element.parentElement).css("height");
        }
        ...
        if (tdClone) {
          ...
          if (editoableElement.classList.contains("table-main-content")) {
            const copyNode = tdClone.cloneNode(false) as HTMLElement;
  ...
  isTableSelected() {
    return (
      !!this.selectArea && this.selectArea.allCol && this.selectArea.allRow
    );
  }
  ...
  showHighlight(area: TableSelectionArea) {
    ...
    this.hideHighlight();
    const colsHeader = this.colsHeader?.find(Template.COLS_HEADER_ITEM_CLASS);
    const rowsHeader = this.rowsHeader?.find(Template.ROWS_HEADER_ITEM_CLASS);
    for (let row = fBeginRow; row <= fEndRow; row++) {
      for (let col = fBeginCol; col <= fEndCol; col++) {
        const cell = tableModel.table[row][col];
        if (this.table.helper.isEmptyModelCol(cell)) {
          if (begin.row > cell.parent.row) begin.row = cell.parent.row;
          if (begin.col >= cell.parent.col) begin.col = cell.parent.col;
          const parent = tableModel.table[cell.parent.row][cell.parent.col];
    ...
    if (end.row === tableModel.rows - 1 && end.col === tableModel.cols - 1) {
    ...
    const headerHeight =
      this.colsHeader?.find(Template.COLS_HEADER_ITEM_CLASS).get<HTMLElement>()
        ?.offsetHeight || 0;
    top += headerHeight;
