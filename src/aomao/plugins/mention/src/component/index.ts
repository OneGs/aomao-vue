...
class Mention<T extends MentionValue = MentionValue> extends Card<T> {
      ...
      if (options?.action) {
        ...
        this.#request = request.ajax({
          ...
          success: (response: any) => {
            const { result, data } = parse ? parse(response) : response;
            if (!result) return;
            resolve(data);
          },
          method: "GET",
  ...
  queryMarks(clone = true) {
    if (!this.#container) return [];
    return this.#container
      .allChildren()
      .filter((child) => child.isElement() && this.editor.node.isMark(child))
  ...
  render(): string | void | NodeInterface {
    ...
    if (value?.name && !this.#container) {
      ...
      this.#container = $(
        `<span class="data-mention-component">@${unescape(name)}</span>`
      );
    ...
    if (!this.#container) {
      ...
      if (
        ...
        this.component?.render(
          this.root,
          editor.trigger("mention:default") || options?.defaultData || []
        );
      }
    }
    // 可编辑下，展示模式
