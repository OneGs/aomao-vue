...
class CollapseComponent implements CollapseComponentInterface {
  ...
  scroll(direction: "up" | "down") {
    ...
    while (true) {
      if (!activeNode)
        activeNode = direction === "up" ? items[items.length - 1] : items[0];
      if (!!activeNode.attributes("data-key") || startNode.equal(activeNode))
        break;
      else
        activeNode = direction === "up" ? activeNode.prev() : activeNode.next();
    }
  ...
  renderRootEmpty() {
    ...
    if (
      ...
      body.length > 0 &&
      (children?.length === 0 ||
        (children?.length === 1 && children.eq(0)?.hasClass("data-scrollbar")))
    ) {
