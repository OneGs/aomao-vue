  ...
  loadImage(id: string, value: ImageValue) {
    ...
    image.onerror = () => {
      if (this.loadCounts[id] <= 3) {
        ...
        delete this.loadCounts[id];
        value.status = "error";
        (value.message = editor.language.get<string>("image", "loadError")),
          editor.card.update(id, value);
      }
    };
  }
    ...
    request.upload(
      {
        ...
        onSuccess: (response, file) => {
          ...
          if (!result.result) {
            card.update<ImageValue>(component.id, {
              status: "error",
              message:
                result.data || language.get<string>("image", "uploadError"),
            });
          } else {
            src = result.data;
          }
        ...
        onError: (error, file) => {
          ...
          card.update<ImageValue>(component.id, {
            status: "error",
            message:
              error.message || language.get<string>("image", "uploadError"),
          });
          delete this.cardComponents[file.uid || ""];
        },
      },
