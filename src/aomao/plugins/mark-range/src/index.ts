...
> extends MarkPlugin<T> {
  ...
  preview(key: string, id?: string) {
    ...
    if (id) {
      ...
      editor.mark.wrap(
        `<${this.tagName} ${
          this.MARK_KEY
        }="${key}" ${DATA_TRANSIENT_ATTRIBUTES}="${this.getPreviewName(key)}" ${
          this.MARK_UUID
        }="${this.m_uuid}" ${this.getPreviewName(key)}="true" />`,
        range
      );
      //遍历当前光标选择节点，拼接选择的文本
      let text = "";
      ...
      subRanges.forEach((subRange) => {
        ...
        if (cardComponent && !cardComponent.executeMark) {
          text += `[card:${(cardComponent.constructor as CardEntry).cardName},${
            cardComponent.id
          }]`;
          if (cardComponent.root.attributes(this.getIdName(key))) return;
          cardComponent.root.attributes(this.MARK_KEY, key);
          cardComponent.root.attributes(this.getPreviewName(key), "true");
        } else {
  ...
  apply(key: string, id: string) {
    const editor = this.editor;
    //遍历预览节点
    editor.container.find(`[${this.getPreviewName(key)}]`).each((markNode) => {
      const mark = $(markNode);
      //获取旧id
      const oldIds = mark.attributes(this.getIdName(key)).trim().split(",");
      //组合新的id串
      ...
      if (oldIds.length > 0) {
        for (let i = 0; i < oldIds.length; i++) {
          ...
          if (parent && oldIds.indexOf(id) < 0 && ids.indexOf(id) < 0) {
            ...
            const oldEnd = oldRange
              ...
              .collapse(false)
              .cloneRange();
            oldRange.setStart(oldBegin.startContainer, oldBegin.startOffset);
            oldRange.setEnd(oldEnd.endContainer, oldEnd.endOffset);
            const reuslt = oldRange.comparePoint(parent, mark.index());
            if (reuslt >= 0) {
              ids.push(id);
        ...
        //未增加就驾到末尾
        if (ids.length === oldIds.length && oldIds.indexOf(id) < 0) {
          ids.push(id);
        }
      } else {
        ids.push(id);
      }
      mark.attributes(DATA_TRANSIENT_ATTRIBUTES, this.getPreviewName(key));
      //设置新的id串
  ...
  remove(key: string, id: string) {
    ...
    const elements: Array<NodeInterface | Node> = this.findElements(key, id);
        ...
        if (id) {
          ...
          if (childNode.isCard()) {
            ...
            if (cardComponent && cardComponent.executeMark) {
              ...
              const markNode = cardComponent.root.find(`[${idName}="${id}"]`);
          ...
          paths.unshift({
            id: id.split(","),
            path: rangePath ? [rangePath.start.path, rangePath.end.path] : [],
          });
        }
      },
      false,
