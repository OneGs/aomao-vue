...
> extends Plugin<T> {
    ...
    if (node.isList(block)) {
      ...
      if (padding > 0) {
        ...
        if (newValue <= 0) block.css(TEXT_INENT_KEY, "");
          ...
          block.css(TEXT_INENT_KEY, addUnit(newValue > 0 ? newValue : 0, "em"));
        }
      }
    }
  }
    ...
    return [
      {
        from: (_, styles, attributes) => {
          return !!styles["padding-left"] || !!attributes[TEXT_INENT_KEY];
        },
        to: (name, styles, attributes) => {
          const node = $(`<${name} />`);
          let valueStr = styles["padding-left"] || attributes[TEXT_INENT_KEY];
          ...
          if (valueStr.endsWith("px")) {
            ...
            if (!fontSize.endsWith("px")) fontSize = "16px";
            const value = removeUnit(valueStr) / removeUnit(fontSize);
  ...
  onShiftTab = (event: KeyboardEvent) => {
    ...
    if (!isEngine(editor)) return;
    event.preventDefault();
    editor.command.execute((this.constructor as PluginEntry).pluginName, "out");
    return false;
  };
